...
function extractCredentials(done) {
    ...
    //Grant type
    //this.grantType = this.req.body && this.req.body.grant_type;//(this is original ) this line has been commented in order to block the granttype by sandun.
    this.grantType = 'password'; // assign the default value by sandun.

    if (!this.grantType || !this.grantType.match(this.config.regex.grantType)) {
        return done(
            error('invalid_request', 'Invalid or missing grant_type parameter')
        );
    }
    ...
    if (
        ...
        return done(
            error('invalid_client', 'Invalid or missing client_id parameter')
        );
...
function credsFromBasic(req) {
    ...
    if (!apireq) return false;

    return new Client(apireq.api_key, apireq.api_secret, apireq.api_client_id);
}
...
function checkGrantType(done) {
    if (
        this.grantType.match(/^[a-zA-Z][a-zA-Z0-9+.-]+:/) &&
...
function usePasswordGrant(done) {
    // User credentials
    var api_key = this.req.body.api_key,
        api_secret = this.req.body.api_secret,
        api_client_id = this.req.body.api_client_id;
    if (!api_key || !api_secret || !api_client_id) {
        return done(
            error(
                'invalid_client',
                'Missing parameters. "api_key" , "api_secret" and "api_client_id" are required'
            )
        );
    }
    ...
    return this.model.getUser(
        ...
        function (err, user) {
            ...
            if (!user) {
                return done(
                    error('invalid_grant', 'User credentials are invalid')
                );
            }
...
function useClientCredentialsGrant(done) {
    ...
    var self = this;
    return this.model.getUserFromClient(
        clientId,
        clientSecret,
        function (err, user) {
            if (err) return done(error('server_error', false, err));
            if (!user) {
                return done(
                    error('invalid_grant', 'Client credentials are invalid')
                );
            }
...
function saveAccessToken(done) {
    ...
    if (this.config.accessTokenLifetime !== null) {
        ...
        expires.setSeconds(
            expires.getSeconds() + this.config.accessTokenLifetime
        );
    }

    this.model.saveAccessToken(
        accessToken,
        this.client.clientId,
        expires,
        this.user,
...
function saveRefreshToken(done) {
    ...
    if (this.config.refreshTokenLifetime !== null) {
        expires = new Date(this.now);
        expires.setSeconds(
            expires.getSeconds() + this.config.refreshTokenLifetime
        );
    }

    this.model.saveRefreshToken(
        refreshToken,
        this.client.clientId,
        expires,
        this.user,
